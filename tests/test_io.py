...
class IOSmokeTest(unittest.TestCase):
    ...
    def test_load_membrane(self):
        ...
        assert isinstance(segm, np.ndarray), \
            "Error in io.load_membrane: Output data not an ndarray."
        assert segm.shape==(50, 50, 50), \
    ...
    def test_load_coords(self):
        ...
        coords_reorder, n_order_reorder = io.load_coords(file_in=self.coords_path, order="xyz")

        assert isinstance(coords, np.ndarray), \
            "Error in io.load_coords: Output data not an ndarray."
        assert coords.dtype==int, \
            "Error in io.load_coords: Output data not integers."
        assert n_order==[0, 1, 2], \
